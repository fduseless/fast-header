...
def test_unicode():
    ...
    assert str(cd) == 'attachment; filename="?????.pdf"; filename*=UTF-8\'\'%D0%BF%D0%BB%D0%B0%D0%BD%D1%8B.pdf'

def test_fallback():
    cd = ContentDisposition(filename='£ and € rates.pdf')
    assert str(cd) == 'attachment; filename="£ and ? rates.pdf"; filename*=UTF-8\'\'%C2%A3%20and%20%E2%82%AC%20rates.pdf'
    cd = ContentDisposition(filename='€ rates.pdf')
    assert str(cd) == 'attachment; filename="? rates.pdf"; filename*=UTF-8\'\'%E2%82%AC%20rates.pdf'

def test_special_chars():
    cd = ContentDisposition(filename='€\'*%().pdf')
    assert str(cd) == 'attachment; filename="?\'*%().pdf"; filename*=UTF-8\'\'%E2%82%AC%27%2A%25%28%29.pdf'

def test_hex():
    cd = ContentDisposition(filename='the%20plans.pdf')
    assert str(cd) == 'attachment; filename="the%20plans.pdf"; filename*=UTF-8\'\'the%2520plans.pdf'

def test_unicode2():
    cd = ContentDisposition(filename='€%20£.pdf')
    assert str(cd) == 'attachment; filename="?%20£.pdf"; filename*=UTF-8\'\'%E2%82%AC%2520%C2%A3.pdf'

def test_fallback_false():
    cd = ContentDisposition(filename='£ and € rates.pdf', fallback=False)
    assert str(cd) == 'attachment; filename*=UTF-8\'\'%C2%A3%20and%20%E2%82%AC%20rates.pdf'
...
def test_str_fallback():
    cd = ContentDisposition(filename='£ and € rates.pdf', fallback ='£ and EURO rates.pdf')
    assert str(cd) == 'attachment; filename="£ and EURO rates.pdf"; filename*=UTF-8\'\'%C2%A3%20and%20%E2%82%AC%20rates.pdf'

def test_str_fallback_ISO_8859():
    cd = ContentDisposition(filename='"£ rates".pdf', fallback ='£ rates.pdf')
    assert str(cd) == 'attachment; filename="£ rates.pdf"; filename*=UTF-8\'\'%22%C2%A3%20rates%22.pdf'
...
def test_basename_fallback():
    ...
    assert str(cd) == 'attachment; filename="EURO rates.pdf"; filename*=UTF-8\'\'%E2%82%AC%20rates.pdf'
...
def test_parse_b2():
    cd = ContentDisposition.parse('attachment; filename="00000000001111111111222222222233333"')
    assert cd.type == 'attachment'
...
def test_parse_36():
    with pytest.raises(Exception) as e_info:
        ContentDisposition.parse('attachment; filename=foo.html, attachment; filename=bar.html')
...
def test_parse_48():
    cd = ContentDisposition.parse('attachment; filename*=UTF-8\'\'foo-%c3%a4-%e2%82%ac.html')
    assert cd.type == 'attachment'
    assert cd.parameters == { 'filename': 'foo-ä-€.html' }
...
def test_parse_51():
    cd = ContentDisposition.parse('attachment; filename*=iso-8859-1\'\'foo-%c3%a4-%e2%82%ac.html')
    assert cd.type == 'attachment'
...
def test_parse_63():
    cd = ContentDisposition.parse('attachment; filename*0="foo"; filename*1="\\b\\a\\r.html"')
    assert cd.type == 'attachment'
    assert cd.parameters == { 'filename*0': 'foo', 'filename*1': 'bar.html' }


def test_parse_64():
    cd = ContentDisposition.parse('attachment; filename*0*=UTF-8\'\'foo-%c3%a4; filename*1=".html"')
    assert cd.type == 'attachment'
...
def test_parse_69():
    cd = ContentDisposition.parse('attachment; filename="foo-ae.html"; filename*=UTF-8\'\'foo-%c3%a4.html')
    assert cd.type == 'attachment'
    assert cd.parameters == { 'filename': 'foo-ä.html' }

def test_parse_70():
    cd = ContentDisposition.parse('attachment; filename*=UTF-8\'\'foo-%c3%a4.html; filename="foo-ae.html"')
    assert cd.type == 'attachment'
    assert cd.parameters == { 'filename': 'foo-ä.html' }


def test_parse_71():
    cd = ContentDisposition.parse('attachment; filename*0*=ISO-8859-15\'\'euro-sign%3d%a4; filename*=ISO-8859-1\'\'currency-sign%3d%a4')
    assert cd.type == 'attachment'
    assert cd.parameters == { 'filename': 'currency-sign=¤', 'filename*0*': 'ISO-8859-15\'\'euro-sign%3d%a4' }
