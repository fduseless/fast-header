...
import os
import re
from typing import ClassVar, Dict, Literal, Self, cast
...
PARAM_REGEXP = re.compile(r""";[\x09\x20]*([!#$%&'*+.0-9A-Z^_`a-z|~-]+)[\x09\x20]*=[\x09\x20]*("(?:[\x20!\x23-\x5b\x5d-\x7e\x80-\xff]|\\[\x20-\x7e])*"|[!#$%&'*+.0-9A-Z^_`a-z|~-]+)[\x09\x20]*""") # g
TEXT_REGEXP = re.compile(r"""^[\x20-\x7e\x80-\xff]+$""")
TOKEN_REGEXP = re.compile(r"""^[!#$%&'*+.0-9A-Z^_`a-z|~-]+$""")

# RegExp for various RFC 5987 grammar
EXT_VALUE_REGEXP = re.compile(r"""^([A-Za-z0-9!#$%&+\-^_`{}~]+)'(?:[A-Za-z]{2,3}(?:-[A-Za-z]{3}){0,3}|[A-Za-z]{4,8}|)'((?:%[0-9A-Fa-f]{2}|[A-Za-z0-9!#$&+.^_`|~-])+)$""")
# RegExp for various RFC 6266 grammar
DISPOSITION_TYPE_REGEXP = re.compile(r"""^([!#$%&'*+.0-9A-Z^_`a-z|~-]+)[\x09\x20]*(?:$|;)""")

def _ustring(text: str) -> str:
    return "UTF-8''" + quote(text, encoding='utf-8')

def _qstring(text: str) -> str:
...
class ContentDisposition(BaseModel, extra="allow"):
        ...
        if self.__pydantic_extra__:
            for k, v in self.__pydantic_extra__.items():
                if not isinstance(v, str):
                    raise ValueError("Only str extra param is supported. Value of {k} is not str")
        return self
    ...
    def parameters(self) -> Dict[str, str]:
        ...
        if fallback_name == self.filename:
            fallback_name = None
        if fallback_name is not None or not is_quoted or HEX_ESCAPE_REGEXP.search(self.filename) is not None:
            ret["filename*"] = self.filename
        if is_quoted or fallback_name is not None:
            ret["filename"] = fallback_name if fallback_name is not None else self.filename
        return ret
    ...
    def __str__(self) -> str:
        ...
        for k, v in sorted(self.parameters.items()):
            val = _ustring(v) if k.endswith("*") else _qstring(v)
            io.write('; ')
            io.write(k)
            io.write('=')
            io.write(val)
